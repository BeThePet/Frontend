...
const shareTwitter = () => {
  const url = encodeURI(window.location.href)
  const text = '나의 반려견의 MBTI는?'
  window.open("https://twitter.com/intent/tweet?text=" + text + "&url=" + url)
}

// 다시 테스트하기는 테스트 페이지에서 처리됨
...
export default function MbtiResultPage() {
  ...
  const handleRetakeTest = () => {
    // 다시 테스트하기 시 페이지를 완전히 새로고침해서 초기 상태로 만듦
        ...
        >
          ...
          <div className="relative">
            <button 
              ...
              className="w-full h-12 rounded-full border-2 border-pink-300 bg-white text-pink-600 text-base font-bold flex items-center justify-center gap-2 shadow-md"
            >
            ...
            {showShareOptions && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="absolute bottom-full left-0 right-0 mb-2 bg-white rounded-lg shadow-lg border border-pink-200 overflow-hidden"
              >
                <button
                  onClick={shareFacebook}
                  className="w-full p-3 text-left hover:bg-pink-50 border-b border-pink-100 font-medium text-gray-700"
                >
                  📘 페이스북으로 공유
                </button>
                <button
                  onClick={shareTwitter}
                  className="w-full p-3 text-left hover:bg-pink-50 font-medium text-gray-700"
                >
                  🐦 트위터로 공유
                </button>
              </motion.div>
          ...
          <button 
            ...
            className="w-full h-12 rounded-full bg-pink-500 text-white text-base font-bold"
          >
            다시 테스트하기
          </button>
        </motion.div>
