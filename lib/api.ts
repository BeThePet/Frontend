...
export const chatbotApi = {
  // 대화방 목록 조회
  getRooms: async () => {
    const response = await apiCallWithRetry('/chatbot/rooms', {
      method: 'GET',
    })
  ...
  deleteRoom: async (roomId: string) => {
    const response = await apiCallWithRetry(`/chatbot/rooms/${roomId}`, {
      method: 'DELETE',
    })
  ...
  getMessages: async (roomId: string) => {
    const response = await apiCallWithRetry(`/chatbot/rooms/${roomId}/messages`, {
      method: 'GET',
    })
  ...
  sendFirstMessage: async (roomId: string, content: string) => {
    const response = await apiCallWithRetry(`/chatbot/rooms/${roomId}/messages/first`, {
      method: 'POST',
  ...
  // 챗봇 헬스체크
  healthCheck: async () => {
    const response = await apiCallWithRetry('/chatbot/health', {
      method: 'GET',
    })
